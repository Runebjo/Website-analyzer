...
export const PostTable = ({ posts, headers }) => {
  ...
  return (
    <div>
      ...
      <div className="flex justify-between align items-end mb-2">
        <div>
          ...
          {displayIndexStatus && (
            <span className="ml-4 text-green-500">
              Indexed: {filteredPosts.filter((f) => !f.isIndexed).length}
            </span>
          )}
          {displayIndexStatus && (
            <span className="ml-4 text-red-500">
              Not Indexed: {filteredPosts.filter((f) => f.isIndexed).length}
            </span>
          )}
          {numOfSerps > 0 && (
            <span className="ml-4"> Number of serps: {numOfSerps}</span>
          )}
        </div>
        <div className="flex">
          <input
            ...
            value={apiKey}
            className="w-64 px-4 py-1 ml-4 leading-normal bg-white border border-gray-300 rounded-lg appearance-none focus:outline-none focus:shadow-outline"
          />
          <button
            className="px-4 py-1 ml-1 border rounded-lg focus:outline-none focus:shadow-outline"
            onClick={() => checkForIndex()}
          >
            Check index
          </button>
          {isLoading && (
            <div className="w-8 h-8 ease-linear border-8 border-t-8 border-gray-200 rounded-full loader inline-block ml-4"></div>
          )}
        </div>
      </div>
        ...
        <tbody>
          {filteredPosts.map((post, index) => (
            ...
            >
              ...
              {displayIndexStatus && (
                <td className="px-4 py-2 border">{post.isIndexed}</td>
              )}
      ...
      {displayModal && (
        <Modal outline={outline} closeModal={closeModal}>
          <div className="m-4">
